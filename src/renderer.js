...
export class Renderer {
  ...
  drawBackground(camera, elapsed) {
    ...
    for (let tileY = startY; tileY <= endY; tileY += 1) {
      for (let tileX = startX; tileX <= endX; tileX += 1) {
        ...
        ctx.fillStyle = isEven ? 'rgba(255,255,255,0.02)' : 'rgba(255,255,255,0.012)';
        ...
        const shimmer = 0.05 + Math.sin(elapsed * 0.5 + runePhase * Math.PI * 2) * 0.015;
  ...
  drawPlayer(player, camera) {
    ...
    this.drawCircle(screen.x, screen.y, player.radius + 6, 'rgba(78, 205, 196, 0.18)', 0.8);
  ...
  drawPickup(pickup, camera) {
    ...
    this.drawCircle(screen.x, screen.y, pickup.radius + 4, 'rgba(78, 205, 196, 0.12)');
  ...
  drawEffect(effect, camera) {
    ...
    if (effect.type === 'arc') {
      ...
      ctx.lineWidth = effect.lineWidth;
      ctx.beginPath();
      ctx.arc(screen.x, screen.y, effect.radius, effect.startAngle, effect.endAngle);
      ctx.stroke();
    }
