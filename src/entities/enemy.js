export class Enemy {
  constructor({ id, x, y, config, scale = 1 }) {
    ...
    this.radius = config.radius * (config.isBoss ? 1 : Math.min(scale, 1.8));
    this.baseRadius = config.radius;
    this.speed = config.speed * Math.min(2.1, 0.92 + scale * 0.25);
    this.maxHealth = Math.round(config.health * scale);
    this.health = this.maxHealth;
    this.damage = Math.round(
      config.damage * Math.min(3.1, 0.98 + scale * 0.28),
    );
